...
if strcmp(Modeflag,'InitializeBlock')
    ...
    for targ_set = 1:length(category_names)
        ...
        Stimuli_sets(targ_set+700) = Preparestimuli(Parameters,stimstruct);
    end
    ...
    distractor_category = dist_set;
    stimstruct.stimsize = paraimagescale;
    Stimuli_sets(dist_set+700) = Preparestimuli(Parameters,stimstruct);
...
numblocks = Numtrials/24/2;
    ...
    critical_distractor = 0;
    
    target_sequence = [Shuffle(1:24)];
    
    counterbalance_cdft = randi(2,1,24);
...
elseif strcmp(Modeflag,'InitializeTrial')
    ...
    critical_distractor_loc = randi(2);
    which_falsetarg_loc = 2 - critical_distractor_loc + 1;
    ...
    next_cat = ~mod(Trial,12);
    
    if next_cat
        current_cat = current_cat + 1;
        critical_distractor = 1;
        target_sequence = Shuffle(1:24);
        counterbalance_cdft = randi(2,1,24);
    end
    
    
    periph_targcategory = target_category + 700;
    periph_ftcategory = ft_category + 700;
    
    current_good_dists = good_dists{target_category};  %gather the good distractors for this category
    %     current_good_dists = Shuffle(current_good_dists);
    shuff_dists = Shuffle(current_good_dists);   %scramble the order
    ...
    loc_left = locx - para_offset;
    loc_right = locx + para_offset;
    ...
    critical_distractor = critical_distractor + 1;
    cd_shown = target_sequence(critical_distractor);
    which_targ_shown = target_sequence(critical_distractor+11);
    
    Trial_Export.which_targ = which_targ_shown;
    
    
    %     ~mod(Trial,(Numtrials/(Numtrials/numblocks)));
    ...
    Trial_Export.critical_distractor = cd_shown;
    Trial_Export.target_category = target_category;
    ...
    pre_falsetarg_time = pre_target_time;
    
    %
    which_targ_time = disp_time + time_interval * (dist_b4_targ + 2);
    
    %     sendmarkerwpause(Parameters,Parameters.MARKERS.FALSETARGTIME + dist_b4_falsetarg - 15); %EEG marker for false target time
    
    %
    cd_or_ft = counterbalance_cdft(critical_distractor);
    
    num_displays = 15;
    
    %Distractors (with target)
    for dist_index = 1:num_displays
        ...
        Events = newevent_show_stimulus(Events,distractor_category+700,safe_distractors(dist_index),loc_left,locy,disp_time + time_interval * dist_index,'screenshot_no','clear_yes'); %distractors (left)
        
        Events = newevent_show_stimulus(Events,distractor_category,safe_distractors(dist_index+(num_displays*2)),locx,locy,disp_time + time_interval * dist_index,'screenshot_no','clear_no'); %distractors (mid)
        
        
        Events = newevent_show_stimulus(Events,distractor_category+700,safe_distractors(dist_index+num_displays),loc_right,locy,disp_time + time_interval * dist_index,'screenshot_no','clear_no'); %distractors (right)
    ...
    if cd_or_ft == 1
        ...
        if critical_distractor_loc == 1
            Events = newevent_show_stimulus(Events,periph_targcategory,cd_shown,loc_left,locy,pre_target_time,'screenshot_no','clear_no'); %target
            ...
            Events = newevent_show_stimulus(Events,periph_targcategory,cd_shown,loc_right,locy,pre_target_time,'screenshot_no','clear_no'); %target
            if pre_target_time ~= pre_falsetarg_time
        ...
        if which_falsetarg_loc == 1
            Events = newevent_show_stimulus(Events,periph_ftcategory,which_falsetarg,loc_left,locy,pre_falsetarg_time,'screenshot_no','clear_no'); %false target
            ...
            Events = newevent_show_stimulus(Events,periph_ftcategory,which_falsetarg,loc_right,locy,pre_falsetarg_time,'screenshot_no','clear_no'); %false target
            if pre_falsetarg_time == pre_target_time
    ...
    Events = newevent_show_stimulus(Events,target_category,which_targ_shown,locx,locy,which_targ_time,'screenshot_no','clear_no'); %mid stream
    Events =  newevent_ParallelPort_mark(Events,which_targ_time,Parameters.MARKERS.MidTarg); %EEG marker for mid stream target
...
%     symbol_time = disp_time + time_interval * (dist_index+1);
    object_question_time = disp_time + time_interval * (dist_index+1);
...
feedback_time = object_answer_time + .01;
    ...
    if see_cdft;mid_stream_clear='clear_no';else mid_stream_clear='clear_yes';end
    
    if cd_or_ft == 1 && see_cdft
        %Target (either left or right)
        if critical_distractor_loc == 1
            Events = newevent_show_stimulus(Events,periph_targcategory,cd_shown,loc_left,locy,feedback_time,'screenshot_no','clear_yes'); %target
        else
            Events = newevent_show_stimulus(Events,periph_targcategory,cd_shown,loc_right,locy,feedback_time,'screenshot_no','clear_yes'); %target
        end
    else
        %False Target (either left or right)
        if which_falsetarg_loc == 1
            Events = newevent_show_stimulus(Events,periph_ftcategory,which_falsetarg,loc_left,locy,feedback_time,'screenshot_no','clear_yes'); %false target
        else
            Events = newevent_show_stimulus(Events,periph_ftcategory,which_falsetarg,loc_right,locy,feedback_time,'screenshot_no','clear_yes'); %false target
        end
    end
    
    Events = newevent_show_stimulus(Events,target_category,which_targ_shown,locx,locy,feedback_time,'screenshot_no',mid_stream_clear); %mid stream
    
%    if symbol_chance(Trial) == 1
%         Events = newevent_show_stimulus(Events,other_sets+1,symbol,locx,locy-200,feedback_time,'screenshot_no','clear_no'); %Symbol feedback
%     else
%         
%     end 
    ...
    sendmarkerwpause(Parameters,Parameters.MARKERS.TARGETSIDE + critical_distractor_loc - 1); %EEG marker for what side the target appeared
    
    sendmarkerwpause(Parameters,Parameters.MARKERS.TARGETTIME + dist_b4_targ - 5); %EEG Marker for target time
    
    sendmarkerwpause(Parameters,Parameters.MARKERS.WHICHPIC + cd_shown - 1); %EEG marker for which target is shown
...
%         end
        if cd_or_ft == 1
            Trial_Export.critical_distractor = cd_shown;
            Trial_Export.false_target_category = 0;
            Trial_Export.false_target = 0;
        else
            Trial_Export.critical_distractor = 0;
            Trial_Export.false_target_category = ft_category;
            Trial_Export.false_target = which_falsetarg;
        end
    end
